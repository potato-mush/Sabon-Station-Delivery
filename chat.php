...
function generateResponse($message, $conn)
{
    ...
    if (
        strpos($message, 'fabric') !== false || strpos($message, 'conditioner') !== false ||
    ...
    if (
        strpos($message, 'discount') !== false || strpos($message, 'sale') !== false ||
        strpos($message, 'savings') !== false || strpos($message, 'percent off') !== false
    ) {

        // Get products with discounts
        $sql = "SELECT productId, productName, productPrice, productDesc 
        FROM products 
        WHERE productDesc LIKE '%discount%' OR productDesc LIKE '%off%' OR 
              productDesc LIKE '%save%' LIMIT 5";
        $result = mysqli_query($conn, $sql);

        if (mysqli_num_rows($result) > 0) {
            $discountProducts = [];
            while ($row = mysqli_fetch_assoc($result)) {
                // Extract discount percentage from description using regex
                if (preg_match('/(\d+)%/', $row['productDesc'], $matches)) {
                    $discountPercent = $matches[1];
                    $originalPrice = $row['productPrice'] / (1 - ($discountPercent / 100));
                    $savings = $originalPrice - $row['productPrice'];

                    $discountProducts[] = "<a href='viewProduct.php?productid={$row['productId']}'>{$row['productName']}</a> - {$discountPercent}% off! Save ₱" . number_format($savings, 2);
                } else {
                    $discountProducts[] = "<a href='viewProduct.php?productid={$row['productId']}'>{$row['productName']}</a>";
                }
            }

            return "Here are our current discounted items: <br>" . implode("<br>", $discountProducts);
        }

        return "Please check our home page regularly for special discounts and promotions!";
    }
